    ...
    <div class="page-breadcrumb breadcrumb-courses">
        ...
        <div class="container">
            <div class="breadcrumb-center d-table">
                ...
                <h3>الإشتراك بالدورة</h3>

                <nav>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{route('site::courses')}}">الدورات</a></li>
                        <li class="breadcrumb-item active">الإشتراك بالدورة</li>
                    </ol>
                </nav>
                ...
                <div class="col-lg-12">
                    ...
                    <div id="no-more-tables" class="table-pay">
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <div class="alert alert-info">السعر لشخص واحد ({{$course->price}} ريال) السعر لأكثر من
                                    شخص ({{$course->price2}} ريال)
                                </div>
                            </div>
                        </div>
                        <table class="col-md-12 table-bordered table-striped table-condensed cf">
                            <thead class="head">
                            <tr>
                                <th class="numeric">الدورة المسجلة</th>
                            ...
                            <tr>
                                ...
                                <td data-title="السعر الإجمالي" class="numeric total"><span
                                        class="font-def priceTotal">{!! $course->price ? intval($course->price) ."<span class='font-def'>ريال</span>" : "<span class='font-def'>مجانا</span>" !!}</span>
                                </td>
                            </tr>
                    ...
                    <div class="payment">
                        ...
                        @if($course->price > 0)
                            <button type="submit" id="payNow" class="complete">{{__('Subscribe now')}}
                                (<span class="priceTotal">{{intval($course->price)}}</span>)
                            </button>
                        @else
                            <form method="post" action="{{route('site::course_order',$course->id)}}">
                                {{csrf_field()}}
    ...
    <script type="text/javascript">
        var Config = {
            settings: {
                ...
                title: "{{$course->name}} ( {{auth()->user()->firstname}} {{auth()->user()->lastname}} ) ",
                currency: "SAR",
                product_names: "{{$course->name}}",
        ...
        $('body').on('change', '#persons', function () {
            if ($(this).val() == 1) {
                $('.morePersons').hide();
                $('.onePerson').show();
                var price = $(this).val() *{{$course->price}};
                ...
                $('.onePerson').hide();
                $('.morePersons').show();
                var price = $(this).val() *{{$course->price2}};
                $(".priceTotal").text(price + "ريال")
                $('#paytabs-express-checkout').attr('data-amount', price)

                var formpersons = '<h4>بيانات المشتركين الاخرين</h4>';
