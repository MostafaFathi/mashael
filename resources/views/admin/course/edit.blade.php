        ...
        <div class="col-sm-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <h6 class="card-subtitle"> {{__('Edit course description')}} </h6>
                            ...
                            <div class="form-group{{ $errors->has('slider') ? ' has-error' : '' }}">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="custom-control custom-checkbox mr-sm-2">
                                        <input type="checkbox" class="custom-control-input" id="slider" type="checkbox"
                                               name="slider"
                                               {{$course->slider > 0 ? "checked" : "" }}  value="1">
                                ...
                                <div class="col-md-12">
                                    <select id="category_id" type="file" class="form-control" name="category_id[]"
                                        ...
                                        @foreach(\App\Category::get() as $cat)
                                            ...
                                            <option value="{{$cat->id}}"
                                            @if($course->categories and $course->categories->pluck('id'))
                                                {{ in_array($cat->id,$course->categories->pluck('id')->toArray()) ? "selected" : "" }}
                                                @endif
                                            >{{$cat->name}}
                                            </option>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select id="trainer_id" type="text" class="form-control" name="trainer_id">
                                        @foreach(\App\Trainer::get() as $type)
                                            <option
                                                value="{{$type->id}}" {{$course->trainer_id == $type->id ? "selected" : ""}}>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select id="type_id" type="text" class="form-control" name="type_id">
                                        @foreach(\App\Type::get() as $type)
                                            <option
                                                value="{{$type->id}}" {{$course->type_id == $type->id ? "selected" : ""}}>{{$type->name}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="custom-control custom-checkbox mr-sm-2">
                                        <input type="checkbox" class="custom-control-input" id="register_status"
                                               ...
                                               {{$course->register_status == 0 ? "checked" : "" }}  value="0">
...
@section('footer')
    ...
    <script type="text/javascript">
        function initialize() {
            var stockholm = new google.maps.LatLng{{$course->coordinates ? $course->coordinates : \App\Setting::getValue('location') }};
            var parliament = new google.maps.LatLng{{$course->coordinates ? $course->coordinates : \App\Setting::getValue('location') }};
